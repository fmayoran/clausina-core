...
row=$(psqlc "SELECT row_to_json(t) FROM (SELECT id,enfasis,canal FROM contenido.solicitudes_propuesta WHERE estado='pendiente' ORDER BY creado_en LIMIT 1) t;")
...
echo "$(ts) pedido $sid enfasis='$enfasis'" >> "$LOG"
...
recientes=$(psqlc "SELECT COALESCE(json_agg(json_build_object('titulo',titulo_interno,'caption',left(caption,200))),'[]'::json) FROM (SELECT pz.titulo_interno, r.caption FROM contenido.piezas pz JOIN contenido.revisiones r ON r.id=pz.revision_vigente WHERE r.estado='publicada' ORDER BY r.publicado_en DESC LIMIT 10) s;")
...
n=$(python3 "$MOTOR/scripts/propuestas_publicar.py" "$CID" "$CHAT" "$BOT" "$canal" 2>>"$LOG")
