...
2. Para cada rechazo:
   ...
   c. **Clasificar el `motivo_rechazo`** (solo Instagram):
      - **ANTES de clasificar — ¿el texto a corregir está EN LA IMAGEN?** Si el motivo pide **quitar / no usar / cambiar / corregir una frase, palabra o dato puntual** (p.ej. "no uses 'Pará. Comé. Seguí.'", "sacá la dirección vieja", "está mal escrito X", "cambiá el horario"), **descargá `asset_ig` y MIRÁ la imagen** antes de decidir: muchas piezas tienen ese texto **horneado en el arte** (no solo en el caption). Si la frase/dato está en la imagen → es **VISUAL editable** (hay que **rehornear el arte sin esa frase**; reescribir solo el caption NO la saca y la pieza vuelve "igual"). Solo si está únicamente en el caption/datos y **no** en la imagen → es **de TEXTO**. Ante la duda, tratalo como VISUAL.
      - **De TEXTO** (copy/caption/título/datos/tono — y la frase **no** está en la imagen): redactar la versión corregida (caption, `web_titulo`, `web_copy`, `web_tags`) **reusando la misma imagen** (`asset_ig` igual). `POST cf-crear-pendiente` **con `pieza_id`** y los textos nuevos (`notas` = qué se corrigió). Con el `{token}` que devuelve: `GET cf-pub-notify?token=<token>`. **No hace falta marcar nada**: la revisión nueva supera a la rechazada y sale de la cola sola.
      - **VISUAL editable** (tipografía/colores fuera de marca, "el texto tapa la comida", "sacá/borrá X", "más cinematográfico", reencuadre, otro texto): **corregilo vos editando la pieza**. Partí de la media base (descargá `asset_ig`), editá la imagen con Higgsfield si hace falta (`nano_banana_2 --image`, ver `scripts/higgsfield/README.md` → *Edición de imagen por instrucción*) y/o rehorneá el texto con la **tipografía de marca** (ver *Texto de marca horneado*). Acondicioná a 9:16 si es Story/Reel, subí (commit+push, verificá 200; el mp4 debe pesar <24MB, tope de Cloudflare) y reenviá como **revisión nueva** con `POST cf-crear-pendiente` **+ `pieza_id`** (incluí el `media` nuevo + `formato` si es story) → `GET cf-pub-notify?token=<token>`. La rechazada sale sola de la cola.
