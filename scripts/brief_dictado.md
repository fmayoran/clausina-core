...
que Fer mandó por Telegram (audio + foto/video opcional). Corre como **Director Creativo de Cortafuego**.
...
- Base n8n: `https://crm-n8n.dhmtev.easypanel.host`. Repo: `/root/claudefolder/marcas/cortafuego` (cwd).

## Tono de marca (leer SIEMPRE)
`contexto/CONTEXTO_MARCA.md` y `contexto/REFERENCIAS_INSTAGRAM.md` y el `CLAUDE.md` del proyecto.
Reglas duras: voseo rioplatense, **sin emojis**, frases cortas e imperativas, tildes/ñ correctos,
**"Av. Valentín Vergara"** completa, **mencionar @ardora.ar** en el caption, hashtags locales
(`#cortafuego #asadorurbano #ranelagh #berazategui`). El slogan "Pará. Comé. Seguí." **NO es genérico**
(es del mediodía express). El sistema ya etiqueta/invita a Collab a @ardora.ar solo.
...
2. **Media**:
   ...
   - **Texto horneado SIEMPRE con tipografía de marca** (Barlow + naranja, vía HTML/Playwright; NO `drawtext`). Receta en `scripts/higgsfield/README.md` → "Texto de marca horneado". Cuidá que el texto **no tape lo importante** (la comida).
...
3. **Redactá** caption (con @ardora.ar + hashtags), `web_titulo`, `web_copy`, `web_tags` con la voz de marca.
