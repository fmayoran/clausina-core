...
2. Por cada rechazo (de la lista): si intentos>=5 → cf-avisar + cf-marcar-procesado. Si no, RUTEÁ por "canal":
   A) canal='instagram' → clasificá el motivo:
      ...
      - VISUAL EDITABLE (tipografía/colores, "el texto tapa la comida", "sacá/borrá X", "más cinematográfico", reencuadre): editá la pieza — Higgsfield nano_banana y/o rehorneá el texto con la tipografía/colores de la marca ($MOTOR/scripts/higgsfield/README.md), acondicioná a 9:16 llenando el cuadro (footage no-9:16 -> fondo difuminado, NUNCA barras negras; conservá el audio original), subí (commit+push, verificá 200; el mp4 debe pesar <24MB, tope de Cloudflare) y reenviá con cf-crear-pendiente + "pieza_id" + el "media" nuevo → cf-pub-notify?token=<token>. El "formato" NO lo cambies salvo que el motivo lo pida: si no lo mandás, se hereda el de la versión anterior. Una historia que vuelve como feed sale recortada por Instagram.
